    ...
    def sample(self, batch_size):
        ...
        return np.array(batch_states), np.array(batch_next_states), np.array(
            batch_actions), np.array(batch_rewards).reshape(
                -1, 1), np.array(batch_dones).reshape(-1, 1)
...
class TD3(object):
    def __init__(self, state_dim, num_actions):
        ...
        self.actor_target = Actor(state_dim, num_actions).to(device)
    ...
    def select_action(self, state):
        ...
        action = np.argmax(self.actor(state).cpu().data.numpy().flatten())
        return action
    ...
    def train(self,
        ...
        for it in range(iterations):
            ...
            logits = self.actor_target(next_state)
            dist = torch.distributions.Categorical(logits=logits)
            next_action = dist.probs.argmax(dim=1).float()
            ...
            if np.random.random() > epsilon:
                next_action = dist.sample().view(-1, 1).float()


            # Step 7: The two Critic targets take each the couple (s’, a’) as input and return two Q-values Qt1(s’,a’) and Qt2(s’,a’) as outputs
            target_Q1, target_Q2 = self.critic_target(next_state, next_action.reshape(-1, 1))
            ...
            if it % policy_freq == 0:
                ...
                dist = torch.distributions.Categorical(logits=logits)
                a = dist.probs.argmax(dim=1).float().reshape(-1, 1)
